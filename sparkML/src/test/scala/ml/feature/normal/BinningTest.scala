...
object BinningTest extends SparkTools{
  ...
  baseDf.withColumn("label", baseDf("d14")).show(10, truncate = 0)

  baseDf.select("m1", "d14")
  //input bin return bins-> Array[Double], bins_count-> Array[Long]
//  val (bucket, xx) = baseDf.select("m1").rdd.map(_.getDouble(0)).histogram(10)
//  bucket.foreach(println(_))

  //input bucket return  bins_count -> Array[Long]
// val binsCount =  baseDf.select("m1").rdd.map(_.getDouble(0)).histogram(bucket)
...
//    .setMinInfoGain(0.05) //一个节点分裂的最小信息增益，值为[0,1]
